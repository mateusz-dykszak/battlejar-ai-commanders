...
- [x] Improve Carrier Kiting: Refine carrier movement to not only avoid enemies and borders but actively maintain maximum distance from all active threats while staying within safe boundaries.
- [x] Implement "Emergency Screen" behavior: If carrier health drops below a threshold or a high threat is very close, force all fighters to orbit the carrier tightly (e.g., 30 units) regardless of AI orders.
- [x] Enhanced AI Strategic Prompting: Update the LLM prompt to include its own carrier's health and the number of active vs docked fighters to allow it to switch between aggressive and defensive mindsets.
- [x] Fighter "Harassment" logic: Add logic for a small group of fighters to stay near enemy carriers to disrupt their fighter launches or intercept newly launched units.
...
- [x] Implement carrier border avoidance tactic:
  ...
  - < 5° (nearly parallel) -> 10 units
  - 5°–45° (shallow) -> 15 units
  - 45°–80° (moderate) -> 20 units
  - 80°–90° (head-on) -> 25 units
  Ensure it's checked first and cannot be overridden.
- [x] Task: Change the carrier border avoidance tactics to trigger at 10, 15, 20 and 25 units.
  - [x] Update `calculateBorderTriggerDistance`
  - [x] Update `calculateCarrierManeuver` safety margin
  - [x] Update `AgenticCommanderTest`
  - [x] Verify kiting logic still works with border avoidance priority.
